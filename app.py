...
filepath = "/root/go-ethereum-recorderfile/build/bin/chainmaker.org/log"
...
def index():
    ...
    df = pd.merge(df_start_commit, df_end_commit, on='block_height')
    df.drop_duplicates(subset='block_height', keep='last', inplace=True)

    df.loc[:, 'block_hash'] = df.loc[:, 'block_hash'].apply(lambda x: shorten_id(x))

    df = df.dropna()

    df.rename(columns={'block_height': '块高'}, inplace=True)
    df.rename(columns={'block_hash': '区块哈希'}, inplace=True)
    df.rename(columns={'block_tx_count': '交易数量'}, inplace=True)

    df = df[['块高', '区块哈希', '交易数量', '打包时刻', '落库时刻']]
    df = df.reindex(columns=['块高', '区块哈希', '交易数量', '打包时刻', '落库时刻'])
    ...
    df_tx_block = read_csv_without_duplicates(filepath + '/tx_delay_end.csv')[['tx_hash', 'block_height', 'measure_time']]
    df_tx = pd.merge(df_in, df_tx_block, on='tx_hash')

    df_tx = df_tx.rename(columns={'measure_time': '离开交易池时刻'})
...
def change_switch():
    ...
    if request.method == "GET":
        ...
        if response.status_code == 200:
            # 将 Go 返回的 YAML 格式内容转换为 Python 对象
            ...
            return jsonify({'status': 'error', 'message': 'Failed to fetch accessconfig'}), 400

    elif request.method == "PUT":
        # 获取请求中的数据（假设是 YAML 格式的数据）
        data = yaml.safe_load(request.data)

        # 发送 PUT 请求更新配置
        response = requests.put(url, data=yaml.dump(data), headers={'Content-Type': 'application/x-yaml'})

        if response.status_code == 200:
            # 更新成功，返回更新后的数据
            updated_data = yaml.safe_load(response.text)
            yaml_data = yaml.dump(updated_data, default_flow_style=False)
            return Response(yaml_data, mimetype='application/x-yaml')
        else:
            return jsonify({'status': 'error', 'message': 'Failed to update accessconfig'}), 400
...
def get_peer_message_throughput():
    ...
    line1 = (
        ...
        .set_global_opts(title_opts=opts.TitleOpts(title="节点收发消息总量-接收"),
                         ...
                         datazoom_opts=[
                             opts.DataZoomOpts(range_start=100-zoom_range, range_end=100, pos_bottom="50%",type_="inside",xaxis_index=0),
...
def get_db_state_write_rate():
    ...
    # 保留6位小数
    new_df['value'] = new_df['value'].apply(lambda x: round(float(x), 6))
    ...
    min_value, max_value = 0, 100
    # 拼接标注字符串
    JsStr = "'数据库平均写入速率：" + str(round(new_df['value'].astype(float).mean(), 6)) + "秒/块'"
...
def get_consensus_clique_cost():
    ...
    bar = (
        ...
        .set_global_opts(title_opts=opts.TitleOpts(title="共识耗时"),
                         toolbox_opts=opts.ToolboxOpts(),
                         visualmap_opts=opts.VisualMapOpts(
                             min_=min_value,
                             max_=max_value,
...
def get_contract_time():
    ...
    # 读取csv文件
    df = read_csv_without_duplicates(filepath + filename,3000)
    if len(df) <= 0:
        return render_template('draw.html', chart="<h2>当前文件尚无数据</h2>")
    ...
    # 保留6位
    new_df['value'] = new_df['value'].apply(lambda x: round(float(x*1000), 6))
    # 将新的DataFrame转换为字典列表
    data = new_df.to_dict('records')
    # 获取value中的最大值最小值
    min_value, max_value = 0, 100
    ...
    bar = (
        ...
        .set_global_opts(title_opts=opts.TitleOpts(title="合约执行时间"),
                         ...
                         yaxis_opts=opts.AxisOpts(name="执行时间/ms"),
                         legend_opts=opts.LegendOpts(pos_left="center"),
                         tooltip_opts=opts.TooltipOpts(
                             formatter=JsCode(
                                 """
...
def get_tx_in_block_tps():
    ...
    df_start = read_csv_without_duplicates(filepath + '/block_commit_duration_start.csv')
    df_start = df_start.drop_duplicates(subset='block_height', keep='first')

    df_end = read_csv_without_duplicates(filepath + '/block_commit_duration_end.csv') # 区块落库时刻

    # df_start['block_height_int'] = df_start['block_height'].astype(int)
    # df_start = df_start.sort_values(by='block_tx_count', ascending=False).drop_duplicates(subset='block_height', keep='first').sort_values(by='block_height_int')
    df_end['block_height_int'] = df_end['block_height'].astype(int)
    df_end = df_end.sort_values(by='block_tx_count', ascending=False).drop_duplicates(subset='block_height', keep='first').sort_values(by='block_height_int')
    ...
    df_start = df_start.rename(columns={'measure_time': 'start_time'})
    df_end = df_end.rename(columns={'measure_time': 'end_time'})
    # df_end = df_end.rename(columns={'block_tx_count': 'count'})

    # 按照block_height合并df_start和df_end
    df = pd.merge(df_start, df_end, on='block_height', how='inner')  # 使用block_height列进行合并
    #df = pd.merge(df, df_count, on='block_height', how='inner')

    # 确保 'block_tx_count_x' 和 'duration' 列是数值类型
    df['block_tx_count'] = pd.to_numeric(df['block_tx_count'], errors='coerce')  # 非数值转为NaN
    df = df[df['block_tx_count'].fillna(0) != 0]

    # 计算时间差
    df['duration'] = df.apply(lambda row: calculate_duration(row['end_time'], row['start_time']), axis=1)
    df['duration'] = pd.to_numeric(df['duration'], errors='coerce')  # 非数值转为NaN

    print(df)

    # 删除 duration 列为 NaN 的行（处理无效数据）
    df = df.dropna(subset=['duration'])
    df = df[df['duration'] > 0]
    
    # 计算块内吞吐量 = 块内交易数 / 耗时
    df['block_tps'] = df['block_tx_count'] / df['duration']
    df['block_tps'] = df['block_tps'].apply(lambda x: round(float(x), 4))
    df = df[df['block_tps'] < 400]
                         ...
                         tooltip_opts=opts.TooltipOpts(
                             formatter=JsCode(
                                 ...
                                 function (params) {
                                     console.log(params);
                                     return '区块高度:'+ params.data.block_height + '</br>' +
                                             '区块哈希:'+ params.data.block_hash_x  + '</br>' +
                                             '块内交易数量:'+ params.data.block_tx_count  + '</br>' +
...
def get_block_validation_efficiency():
    ...
    bar = (
        ...
        .set_global_opts(title_opts=opts.TitleOpts(title="区块验证效率"),
                         ...
                         datazoom_opts=[
                             opts.DataZoomOpts(range_start=40, range_end=40.5),
                         ],
                         xaxis_opts=opts.AxisOpts(name="区块高度"),
